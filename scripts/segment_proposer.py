...
parser.add_argument('--detector_dir', default="/mounted_models/singular_object_test")
...
if __name__ == "__main__":
    ...
    for sample in ds_list[0].iter_samples(progress="verbose"):
        for gt_detection in sample.ground_truth.detections:
            gt_detection.label = "trash"
        sample.save()

    session = fo.launch_app(ds_list[0], remote=True)
    fix_crowd_attribute(ds_list[0], value=1)
    color_by_instance(ds_list[0], session)

    for dataset in ds_list:
        dataset_export_path = os.path.join(EXPORT_DIR, dataset.name)
